...
type ParticipantsApprovedChartProps = {
    eventId: string;
    className?: string;
};

function ParticipantsApprovedChart({ eventId, className }: ParticipantsApprovedChartProps) {
    const { t } = useTranslation();
    const { provision: _provision, loading: loadingProvision, isError } = useUserProvision(eventId);

    const chartConfig = {
        progress: { label: t("approved"), color: "var(--chart-2)" },
        remaining: { label: t("remaining"), color: "var(--chart-5)" }
    } satisfies ChartConfig;
    const { stats, loading: loadingStats } = useGetParticipantsStats(eventId);

    if (loadingProvision || loadingStats) return <Skeleton className="h-[530px] w-full" />;
        ...
        return (
            <Empty className="h-[535px] w-full border border-dashed">
                <EmptyHeader>
                    ...
                    <EmptyDescription>{t("participants-data-not-found")}</EmptyDescription>
                </EmptyHeader>
            </Empty>
        );
    }

    const registeredCount = stats?.participantCount || 0;
    const approvedCount = stats?.approvedParticipantCount || 0;
        ...
        <Card className="gap-0 rounded-md py-0 shadow-none">
            <CardHeader className="flex flex-col items-stretch border-b p-0!">
                <div className="flex flex-1 flex-col justify-center gap-1 p-6 pb-4">
                    <CardTitle>{t("analytics-participants")}</CardTitle>
                    <CardDescription>{t("analytics-participants-description")}</CardDescription>
                </div>
                <div className="flex">
...
export default ParticipantsApprovedChart;

...
type AttendeesAttendedChartProps = {
    eventId: string;
    className?: string;
};

function AttendeesAttendedChart({ eventId, className }: AttendeesAttendedChartProps) {
        ...
        return (
            <Empty className="h-[535px] w-full border border-dashed">
                <EmptyHeader>
                    ...
                    <EmptyDescription>{t("attendees-attended-data-not-found")}</EmptyDescription>
                </EmptyHeader>
            </Empty>
        );
    }

    const attendedCount = stats?.attendedAttendees || 0;
    const registeredCount = stats?.registeredAttendees || 0;
    const percentage = registeredCount > 0 ? Math.round((attendedCount / registeredCount) * 100) : 0;
        ...
        <Card className="gap-0 rounded-md py-0 shadow-none">
            <CardHeader className="flex flex-col items-stretch border-b p-0!">
                <div className="flex flex-1 flex-col justify-center gap-1 p-6 pb-4">
                    <CardTitle>{t("analytics-attendees")}</CardTitle>
                    <CardDescription>{t("analytics-attendees-description")}</CardDescription>
                </div>
                <div className="flex">
...
export default AttendeesAttendedChart;

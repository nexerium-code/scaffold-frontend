...
import { useUpdateAttendanceFormFieldsExclusives } from "@/hooks/attendance/useUpdateAttendanceFormFieldsExclusives";
import { useUserAttendance } from "@/hooks/attendance/useUserAttendance";
import { FormInputTypes } from "@/lib/enums";
import { AttendanceFormFieldsExclusivesSchema, AttendanceFormFieldsExclusivesSchemaType } from "@/services/attendance/Attendance.formFields.schemas";
import { initAttendanceFormFieldsExclusivesData, prepareAttendanceFormFieldsExclusivesData } from "@/services/attendance/Attendance.helpers";
import { zodResolver } from "@hookform/resolvers/zod";
import { VisuallyHidden } from "@radix-ui/react-visually-hidden";

function UpdateAttendanceFormFieldsExclusivesDialog() {
    ...
    const { updateAttendanceFormFieldsExclusives, loading } = useUpdateAttendanceFormFieldsExclusives(() => setOpen(false));
    ...
    useEffect(() => {
        if (attendance) form.reset(initAttendanceFormFieldsExclusivesData(attendance), { keepDefaultValues: false });
    }, [attendance, form]);
    ...
    function onSubmit(values: AttendanceFormFieldsExclusivesSchemaType) {
        const prepareData = prepareAttendanceFormFieldsExclusivesData(values);
        updateAttendanceFormFieldsExclusives({ eventId: selectedEvent, payload: prepareData });
    }
            ...
            <DialogContent onCloseAutoFocus={resetState} className="inset-0 h-screen w-screen max-w-none translate-x-0 translate-y-0 gap-0 rounded-none border-none p-0 sm:max-w-none rtl:translate-x-0">
                ...
                <SidebarProvider className="flex min-h-0 flex-col gap-0">
                    <header className="flex h-12 shrink-0 items-center gap-2 border-b p-4">
                        ...
                        <span className="text-muted-foreground text-sm wrap-break-word">{t("update-exclusives-form")}</span>
                    </header>
...
export default UpdateAttendanceFormFieldsExclusivesDialog;

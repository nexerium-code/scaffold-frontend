...
type ExclusivesTableSearchProps<TData> = {
    table: Table<TData>;
    searchFields: Record<string, FormInputSchemaType>;
};

function getFieldTitle(field: FormInputSchemaType, language: string) {
    return language === "en-US" ? field.titleEN || field.titleAR : field.titleAR || field.titleEN;
}

function ExclusivesTableSearch<TData>({ table, searchFields }: ExclusivesTableSearchProps<TData>) {
    ...
    const defaultTarget = currentFilters.find((filter) => Object.keys(searchFields).includes(filter.id))?.id || Object.keys(searchFields)[0];
    ...
    function handleSearch(query: string) {
        table.setColumnFilters((oldFilters) => {
            ...
            const otherFilters = oldFilters.filter((filter) => !Object.keys(searchFields).includes(filter.id));
                ...
                <DropdownMenu>
                    <DropdownMenuTrigger asChild>
                        <InputGroupButton variant="ghost" className="pe-1.5! text-xs">
                            {searchFields[target] ? getFieldTitle(searchFields[target], i18n.language) : target} <ChevronDownIcon className="size-3" />
                        </InputGroupButton>
                    </DropdownMenuTrigger>
                        ...
                        <DropdownMenuRadioGroup value={target} onValueChange={handleTarget}>
                            {Object.entries(searchFields).map(([key, field]) => (
                                <DropdownMenuRadioItem key={key} value={key}>
                                    {getFieldTitle(field, i18n.language)}
...
export default ExclusivesTableSearch;
